...
class TreeNode extends vscode.TreeItem {
    constructor(nodeType, label, collapsible, conn, sectionKind) {
...
class ConnTreeProvider {
    ...
    getChildren(el) {
        const all = this.ctx.globalState.get("pg.connections", []);
        if (!el) {
            return [
        ...
        if (el.nodeType === "section" && el.sectionKind === "vanna") {
            ...
            if (!list.length)
                return [new TreeNode("empty", "Click + to add Vanna connection", vscode.TreeItemCollapsibleState.None)];
        ...
        if (el.nodeType === "section" && el.sectionKind === "postgres") {
            ...
            if (!list.length)
                return [new TreeNode("empty", "Click + to add PostgreSQL connection", vscode.TreeItemCollapsibleState.None)];
            return list.map(c => new TreeNode("conn", c.label, vscode.TreeItemCollapsibleState.None, c));
        }
        return [];
    }
    ...
    panel.webview.onDidReceiveMessage(async (msg) => {
        if (msg.command === "saveVanna") {
            const conn = {
                ...
                dbType: msg.data.dbType || "postgres",
            };
            try {
...
function openQueryPanel(context, node) {
    ...
    panel.webview.html = getQueryRunnerHtml(conn.label, conn.type, settings.fontSize, settings.queryLimit, conn.vannaUrl, dbLabel);
    panel.webview.onDidReceiveMessage(async (msg) => {
        ...
        if (msg.command === "exportCSV") {
            ...
            const csv = [
                columns.join(","),
                ...rows.map((r) => r.map((v) => v == null ? "" : `"${String(v).replace(/"/g, '""')}"`).join(","))
            ].join("\n");
...
// VANNA API
// ─────────────────────────────────────────────────────────────
async function askVannaAPI(baseUrl, query, dbType = "postgres") {
    ...
    const components = json.response || [];
    let sql = "";
    let text = "";
    let columns;
    let rows;
        ...
        if (compType === "data") {
            ...
            if (c && Array.isArray(c.columns) && Array.isArray(c.rows) && c.columns.length > 0) {
                ...
                if (c.rows.length > 0 && Array.isArray(c.rows[0])) {
                    ...
                    rows = c.rows.map((row) => columns.map(col => row[col] ?? null));
                }
                else {
                    rows = c.rows;
                }
                ...
                rows = c.map((row) => columns.map(col => row[col] ?? null));
            }
        }
            ...
            if (!rich) {
                ...
                if (typeof c === "string" && c.trim() && !text) {
                    ...
                    if (typeof c.text === "string" && c.text.trim() && !text) {
                        text = c.text.trim();
                    }
                    else if (typeof c.message === "string" && c.message.trim() && !text) {
                        text = c.message.trim();
                    }
                    else if (typeof c.content === "string" && c.content.trim() && !text) {
                        text = c.content.trim();
                    }
                }
                continue;
            ...
            if (richType === "dataframe" && Array.isArray(rich.columns) && Array.isArray(rich.rows)) {
                ...
                if (rich.rows.length > 0 && Array.isArray(rich.rows[0])) {
                    ...
                    rows = rich.rows.map((row) => columns.map(col => row[col] ?? null));
                }
            }
                ...
                if (Array.isArray(rich.rows) && Array.isArray(rich.columns) && !columns) {
                    ...
                    rows = rich.rows.length > 0 && Array.isArray(rich.rows[0])
                        ? rich.rows
                        : rich.rows.map((row) => columns.map(col => row[col] ?? null));
                }
...
// ─────────────────────────────────────────────────────────────
function getQueryRunnerHtml(label, connType, fontSize, queryLimit, vannaUrl, dbLabel) {
